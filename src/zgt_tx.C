...
void *aborttx(void *arg)
{
  ...
  //write your code
  start_operation(node->tid, node->count);
  do_commit_abort(node->tid,TR_ABORT); //to abort
  finish_operation(node->tid);
  pthread_exit(NULL);			
}
...
void *committx(void *arg)
{
  ...
  //write your code
  start_operation(node->tid, node->count);  
  do_commit_abort(node->tid,TR_END); //to commit
  finish_operation(node->tid);
  pthread_exit(NULL);			
}
...
{
  // write your code
   switch (status) //to check the status of the transaction
    {
    case TR_END:
  ...
  //to check if the transaction is null or not, to check for wait and release the lock
    if(tx == NULL)
    {
    ...
    {
          tx->free_locks(); //free all the locks held by that transaction on all objects 
          tx->remove_tx(); //remove the transaction
          int twait=zgt_nwait(t); //obtaining transactions that are in wait state and storing it in a variable
          while(twait>0) 
          {
            zgt_v(t);   //  releasing semaphore
            printf("\n Released  \n\n ");  
          }
    } 
...
zgt_hlink *tpointer=ZGT_Ht->find(sgno1,obno1);
...
else if (tx->tid==tpointer->tid)
{
  tx->perform_readWrite(tid1,obno1,lockmode1);
}
else
  {
          // requesting types of locks
          // checking the lockmode1
          if(lockmode1 == 'X') 
            { 
                tx->lockmode=lockmode1;
                tx->status = TR_WAIT;
                tx->setTx_semno(tpointer->tid,tpointer->tid); // to set semaphore to tid
                zgt_p(tpointer->tid); // to lock 
                tx->lockmode= ' ';
                tx->status = TR_ACTIVE;
			          tx->perform_readWrite(tid1, obno1, lockmode1);
                zgt_v(tpointer->tid); // to release semaphore for this transaction
                //printf("exit1");
            }
            else if (lockmode1 == 'S' ) 
            {
              if(tpointer->lockmode == 'X' )
              {
                tx->status = TR_WAIT;
                tx->setTx_semno(tpointer->tid,tpointer->tid); // to set semaphore to tid
                zgt_p(tpointer->tid); // to lock 
            ...
            else // when no transaction has shared lock except for this transaction
            {
                tx->perform_readWrite(tid1, obno1, lockmode1);
            }
  }
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode)
{
  ...
  // write your code
    ...
    if(lockmode == 'S')		  //read mode, shared lock
    {   
      ZGT_Sh->objarray[obno]->value=o_val-1;	 // update obj val (o_val)
      fprintf(ZGT_Sh->logfile,"T-%d\t\tReadTx\t\t\t%d:%d:%d\tRead_Lock\tGranted\t\t %c\n", 
          this->tid,obno, ZGT_Sh->objarray[obno]->value, ZGT_Sh->optime[tid],this->status);
    }
    else                  // write mode, exclusive lock
    { 
        ZGT_Sh->objarray[obno]->value=o_val+1;	   // update obj val (o_val)         
        fprintf(ZGT_Sh->logfile,"T-%d\t\tWriteTx\t\t\t%d:%d:%d\tWrite_Lock\tGranted\t\t %c\n", 
          this->tid,obno, ZGT_Sh->objarray[obno]->value, ZGT_Sh->optime[tid],this->status);
    }
}
